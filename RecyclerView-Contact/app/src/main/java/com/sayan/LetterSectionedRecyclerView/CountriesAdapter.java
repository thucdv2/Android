...
public class CountriesAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public static final int LAST_ITEM_VIEW = 2;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == LAST_ITEM_VIEW) {
            return new ItemViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_name_last_item_group, parent, false), context);
        }
    ...
    public int getItemViewType(int position) {
        if (mCountriesModelList.get(position).isSection==1) {
            return SECTION_VIEW;
        } else if (mCountriesModelList.get(position).isSection==0) {
            return CONTENT_VIEW;
        }else {
