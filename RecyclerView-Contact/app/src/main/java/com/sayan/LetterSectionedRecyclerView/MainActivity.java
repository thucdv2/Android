...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int j = 0; j < mCountries.length ; j++) {
            countriesModels.add(new CountriesModel(mCountries[j].toString(),0));
        }
    ...
    private void getHeaderListLatter(ArrayList<CountriesModel> usersList) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (!TextUtils.equals(lastHeader, header)) {
                lastHeader = header;
                if (i > 0) {
                    mSectionList.get(mSectionList.size()-1).isSection =2;
                }
                mSectionList.add(new CountriesModel(header,1));
            }
